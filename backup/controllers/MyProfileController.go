...
func MyProfileController(c echo.Context) error {
	...
	row := db.Table("v_get_user").Where("id = ?", requested_id).Select("id_setting_grup, full_name, gender, email, telephone, address,  username, name_grup, status").Row() 
	err := row.Scan(&id_setting_grup, &full_name, &gender, &email, &telephone, &address, &username, &name_grup, &status)
	if err != nil {
		logs.Println(err)
...
func GetDataProfileController(c echo.Context) error{
	...
	data_images := db.Table("tb_setting_user").Where("id = ?", id_users).Select("full_name, email, telephone, address, image, additional").Row() 
	err = data_images.Scan(&full_name, &email, &telephone, &address, &image, &additional)
	if err != nil {
		logs.Println(err)
	}
